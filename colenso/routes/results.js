...
function basicSearch(searchTerms){
  ...
  <results>{
    ...
    let $context := ft:extract($match[. contains text 'Darwin'], "b")
...
router.get('/', function(req, res, next) {
  ...
  client.execute(query, function(error, result){
    if (!error) {
      ...
      res.render('results', {query: req.query.q, data: formattedResults, results: formattedResults.length});
